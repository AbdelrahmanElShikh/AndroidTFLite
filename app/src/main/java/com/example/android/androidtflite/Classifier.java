...
import static com.example.android.androidtflite.ModelConfig.CLASSIFICATION_THRESHOLD;
import static com.example.android.androidtflite.ModelConfig.MAX_CLASSIFICATION_RESULTS;
...
//                (lhs, rhs) -> Float.compare(rhs.confidence, lhs.confidence)
    private List<Classification> getSortedResult(float[][] resultsArray) {
        PriorityQueue<Classification> sortedResults = new PriorityQueue<>(
                MAX_CLASSIFICATION_RESULTS, new Comparator<Classification>() {
            @Override
